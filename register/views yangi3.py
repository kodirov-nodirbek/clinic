...
class BemorModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if qidiruv:
            queryset = (
                queryset.filter(ism__icontains=qidiruv)
                | queryset.filter(familiya__icontains=qidiruv)
                | queryset.filter(sharif__icontains=qidiruv)
                | queryset.filter(tel__icontains=qidiruv)
            )
        if sana and joylashgan and tolov_sana:
            tolovlar = (
                Tolov.objects.filter(sana=tolov_sana).values("bemor_id").distinct()
            )
            queryset = queryset.filter(joylashgan=True) | queryset.filter(
                royhatdan_otgan_sana=sana
            )
        ...
        if j_id and tolandi is not None and t_date:
            ...
            tolovlar = (
                Tolov.objects.filter(tolandi=False, joylashtirish_id__isnull=False)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(sana=t_date, joylashtirish_id__isnull=False)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    tolangan_sana=t_date, joylashtirish_id__isnull=False
                )
                .values("bemor_id")
                .distinct()
            )
            ...
            tolovlar = (
                Tolov.objects.filter(tolandi=False, yollanma_id__isnull=False)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(sana=t_date, yollanma_id__isnull=False)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(tolangan_sana=t_date, yollanma_id__isnull=False)
                .values("bemor_id")
                .distinct()
            )
            queryset = Bemor.objects.none()
            ...
            tolovlar = (
                Tolov.objects.filter(sana=t_date, joylashtirish_id__isnull=True)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    tolangan_sana=t_date, joylashtirish_id__isnull=True
                )
                .values("bemor_id")
                .distinct()
            )
            ...
            tolovlar = (
                Tolov.objects.filter(sana=t_date, yollanma_id__isnull=True)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(tolangan_sana=t_date, yollanma_id__isnull=True)
                .values("bemor_id")
                .distinct()
            )
            queryset = Bemor.objects.none()
            ...
            tolovlar = (
                Tolov.objects.filter(sana=t_date, yollanma_id__qayerga=qayerga)
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    tolangan_sana=t_date, yollanma_id__qayerga=qayerga
                )
                .values("bemor_id")
                .distinct()
            )
            if qayerga2:
                tolovlar = (
                    tolovlar
                    | Tolov.objects.filter(sana=t_date, yollanma_id__qayerga=qayerga2)
                    .values("bemor_id")
                    .distinct()
                    | Tolov.objects.filter(
                        tolangan_sana=t_date, yollanma_id__qayerga=qayerga2
                    )
                    .values("bemor_id")
                    .distinct()
                )
            ...
            tolovlar = (
                Tolov.objects.filter(
                    yollanma_id__qayerga=qayerga,
                    ozgartirilgan_sana=ozgartirilgan_sana,
                    xulosa_holati=xulosa_holati,
                )
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
            ...
            tolovlar = (
                Tolov.objects.filter(
                    yollanma_id__qayerga=qayerga, xulosa_holati=xulosa_holati
                )
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    xulosa_holati=xulosa_holati, yollanma_id__qayerga=qayerga
                )
                .values("bemor_id")
                .distinct()
            )
            if qayerga2:
                tolovlar = (
                    tolovlar
                    | Tolov.objects.filter(
                        yollanma_id__qayerga=qayerga2, xulosa_holati=xulosa_holati
                    )
                    .values("bemor_id")
                    .distinct()
                    | Tolov.objects.filter(
                        xulosa_holati=xulosa_holati, yollanma_id__qayerga=qayerga2
                    )
                    .values("bemor_id")
                    .distinct()
                )
            ...
            tolovlar = (
                Tolov.objects.filter(
                    sana__range=(t_from_date, t_to_date), joylashtirish_id__isnull=True
                )
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    tolangan_sana__range=(t_from_date, t_to_date),
                    joylashtirish_id__isnull=True,
                )
                .values("bemor_id")
                .distinct()
            )
            ...
            tolovlar = (
                Tolov.objects.filter(
                    sana__range=(t_from_date, t_to_date), yollanma_id__isnull=True
                )
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
                    tolangan_sana__range=(t_from_date, t_to_date),
                    yollanma_id__isnull=True,
                )
                .values("bemor_id")
                .distinct()
            )
            ...
            tolovlar = (
                Tolov.objects.filter(
                    sana__range=(t_from_date, t_to_date), yollanma_id__qayerga=qayerga
                )
                .values("bemor_id")
                .distinct()
                | Tolov.objects.filter(
            ...
            if qayerga2:
                tolovlar = (
                    tolovlar
                    | Tolov.objects.filter(
                        sana__range=(t_from_date, t_to_date),
                        yollanma_id__qayerga=qayerga2,
                    )
                    .values("bemor_id")
                    .distinct()
    ...
    def tolovlar(self, request, pk):
        tolovlar = Tolov.objects.filter(
            joylashtirish_id__isnull=False, joylashtirish_id__ketish_sanasi__isnull=True
        )
        for tolov in tolovlar:
...
class TolovModelViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if (
            tolov.bemor_id.joylashgan == True
            and tolov.joylashtirish_id is not None
            and tolov.joylashtirish_id.ketish_sanasi is None
        ):
        ...
        return Response(
            {
                "success": True,
                "xabar": "To'lov va unga tegishli joylashtirish o'chirildi",
            }
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if bemor and (tolandi is not None or t_date or date):
            ...
            queryset = (
                queryset.filter(bemor_id__id=int(bemor))
                | queryset.filter(sana=tolov_date)
                | queryset.filter(tolangan_sana=tolov_date)
            )
            if tolandi == "false":
                queryset = queryset.filter(tolandi=False)
            else:
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            sub_ids = valid_data.get("subyollanma_idlar", [])
            cost_list = SubYollanma.objects.filter(id__in=sub_ids).values_list(
                "narx", flat=True
            )
            total_sum = sum([*filter(lambda cost: bool(cost), cost_list), 0])

            output["summa"] = total_sum

            paid_amount = valid_data.get("tolangan_summa", [])
            total_sum += sum(
                [*map(lambda payment: getattr(payment, "summa"), paid_amount), 0]
            )

            total_sum += valid_data.get("chegirma")
            summ = valid_data.get("summa")

            payment_status = {"tolandi": total_sum == summ, "haqdor": total_sum > summ}
            serializer.save(xulosa_holati="Topshirilyapti", **payment_status)
        except Exception:
    ...
    def update(self, request, *args, **kwargs):
        ...
        if tolov.get("xulosa_holati"):
            ...
            if serializer.is_valid():
                ...
                return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
        return Response(
            {"xabar": "Ma'lumotda kamchilik bor!"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class XulosaModelViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if bemori:
            tolovlar = Tolov.objects.filter(
                bemor_id__in=Bemor.objects.filter(id=int(bemori))
            )
...
class JoylashtirishModelViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if (qarovchi_bor and xona.bosh_joy_soni < 2) or (
            qarovchi_bor == False and xona.bosh_joy_soni < 1
        ):
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                patient.save()
                boshi = datetime.datetime.strptime(str(joy.kelish_sanasi), "%Y-%m-%d")
                oxiri = datetime.datetime.strptime(
                ...
                if joy.qarovchi:
                    ...
                    s = int(joy.yotgan_kun_soni) * int(joy.xona_id.joy_narxi)
                joy.save()
                Tolov.objects.create(bemor_id=patient, joylashtirish_id=joy, summa=s)
                if qarovchi_bor:
                    xona.bosh_joy_soni -= 2
                else:
                    xona.bosh_joy_soni -= 1
            ...
            with transaction.atomic():
                ...
                if joylashtirish.qarovchi:
                    if joylashtirish.ketish_sanasi:
                        xona.bosh_joy_soni += 2
                    s = (
                        int(joylashtirish.yotgan_kun_soni)
                        * int(joylashtirish.xona_id.joy_narxi)
                        * 2
                    )
                else:
                    if joylashtirish.ketish_sanasi:
                        xona.bosh_joy_soni += 1
                    s = int(joylashtirish.yotgan_kun_soni) * int(
                        joylashtirish.xona_id.joy_narxi
                    )
                xona.save()
                tolov.summa = s
                tolanganlar = 0
            ...
            with transaction.atomic():
                ...
                for i in tolovlar:
                    ...
                    if tolov.tolandi != True:
                        tolov.tolangan_summa.append(
                            {
                                "summa": int(i.get("summa")),
                                "sana": check_to_be_created.get("sana"),
                            }
                        )
                        ...
                        if tolov.yollanma_id:
                            ...
                            if (
                                t == tolov.summa
                                and tolov.joylashtirish_id.ketish_sanasi is not None
                            ):
                                tolov.tolandi = True
                            elif t < tolov.summa:
                                tolov.tolandi = False
...
class TolovlarAPIView(APIView):
    ...
    def get(self, request):
        ...
        if search:
            queryset = (
                queryset.filter(bemor_id__ism__icontains=search)
                | queryset.filter(bemor_id__familiya__icontains=search)
                | queryset.filter(bemor_id__sharif__icontains=search)
                | queryset.filter(bemor_id__tel__icontains=search)
            )
            if yollanma:
        ...
        if from_date and to_date and search_word:
            queryset = queryset.filter(
                sana__range=(from_date, to_date),
                yollanma_id__qayerga=search_word,
                tolangan_sana__isnull=True,
            ) | queryset.filter(
                tolangan_sana__range=(from_date, to_date),
                yollanma_id__qayerga=search_word,
            )
            ...
            queryset = queryset.filter(
                ...
                yollanma_id__qayerga=search_word,
                tolangan_sana__isnull=True,
            ) | queryset.filter(tolangan_sana=by_date, yollanma_id__qayerga=search_word)
        elif from_date and to_date and joylashtirish:
            queryset = queryset.filter(
                sana__range=(from_date, to_date),
                joylashtirish_id__isnull=False,
                tolangan_sana__isnull=True,
            )
            ...
            queryset = queryset.filter(
                sana=by_date, joylashtirish_id__isnull=False, tolangan_summa=[]
            )
            all_objects = Tolov.objects.filter(joylashtirish_id__isnull=False)
            tolovs = Tolov.objects.filter(id=None)
            for obj in all_objects:
            ...
            queryset = queryset.filter(
                sana=by_date, yollanma_id__isnull=False, tolangan_sana__isnull=True
            ) | queryset.filter(tolangan_sana=by_date, yollanma_id__isnull=False)
            ...
            queryset = queryset.filter(
                tolov_qaytarildi=True, joylashtirish_id__isnull=False
            )
        if tolandi is not None:
            if tolandi == "false":
                queryset = queryset.filter(tolandi=False)
            ...
            for tolov in queryset:
                ...
                for i in tolov.tolangan_summa:
                    if i.get("summa") is not None:
                        t += i.get("summa")
                    if (
                        i.get("summa") is not None
                        and from_date <= i.get("sana") <= to_date
                    ):
                        sanadagi += i.get("summa")
                if t <= tolov.summa:
                    qarzdorlik = qarzdorlik + tolov.summa - t
...
class TolovDeleteAPIView(APIView):
    ...
    def delete(self, request, pk):
        ...
        return Response(
            {"succes": "true", "message": "To'lov o'chirildi"},
            status=status.HTTP_200_OK,
        )
