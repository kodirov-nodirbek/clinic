...
class BemorModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if j_id and tolandi is not None and t_date:
            ...
            tolovlar = Tolov.objects.filter(tolandi=False, joylashtirish_id__isnull=False).values(
            ...
            tolovlar = Tolov.objects.filter(tolandi=False, yollanma_id__isnull=False).values(
                "bemor_id").distinct() | Tolov.objects.filter(
            ...
            tolovlar = Tolov.objects.filter(sana=t_date, yollanma_id__isnull=True).values(
                "bemor_id").distinct() | Tolov.objects.filter(
            ...
            tolovlar = Tolov.objects.filter(sana=t_date, yollanma_id__qayerga=qayerga).values(
                "bemor_id").distinct() | Tolov.objects.filter(
            ...
            tolovlar = Tolov.objects.filter(yollanma_id__qayerga=qayerga, ozgartirilgan_sana=ozgartirilgan_sana,
                                            xulosa_holati=xulosa_holati).values(
                "bemor_id").distinct() | Tolov.objects.filter(
                xulosa_holati=xulosa_holati, ozgartirilgan_sana=ozgartirilgan_sana,
                yollanma_id__qayerga=qayerga).values("bemor_id").distinct()
            if qayerga2:
                tolovlar = tolovlar | Tolov.objects.filter(yollanma_id__qayerga=qayerga2,
            ...
            tolovlar = Tolov.objects.filter(yollanma_id__qayerga=qayerga, xulosa_holati=xulosa_holati).values(
                "bemor_id").distinct() | Tolov.objects.filter(
            ...
            tolovlar = Tolov.objects.filter(sana__range=(t_from_date, t_to_date), joylashtirish_id__isnull=True).values(
                "bemor_id").distinct() | Tolov.objects.filter(
                tolangan_sana__range=(t_from_date, t_to_date), joylashtirish_id__isnull=True).values(
                "bemor_id").distinct()
            queryset = Bemor.objects.none()
            for tolov in tolovlar:
                queryset = queryset | Bemor.objects.filter(id=tolov.get('bemor_id'))
        elif t_from_date and t_to_date and j_id:
            tolovlar = Tolov.objects.filter(sana__range=(t_from_date, t_to_date), yollanma_id__isnull=True).values(
                "bemor_id").distinct() | Tolov.objects.filter(
            ...
            tolovlar = Tolov.objects.filter(sana__range=(t_from_date, t_to_date),
                                            yollanma_id__qayerga=qayerga).values(
                "bemor_id").distinct() | Tolov.objects.filter(
                tolangan_sana__range=(t_from_date, t_to_date), yollanma_id__qayerga=qayerga).values(
                "bemor_id").distinct()
            if qayerga2:
                tolovlar = tolovlar | Tolov.objects.filter(sana__range=(t_from_date, t_to_date),
                                                           yollanma_id__qayerga=qayerga2).values(
                    "bemor_id").distinct() | Tolov.objects.filter(
                    tolangan_sana__range=(t_from_date, t_to_date), yollanma_id__qayerga=qayerga2).values(
                    "bemor_id").distinct()
            queryset = Bemor.objects.none()
            for tolov in tolovlar:
                queryset = queryset | Bemor.objects.filter(id=tolov.get('bemor_id'))
...
class TolovModelViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if bemor and (tolandi is not None or t_date or date):
            ...
            queryset = queryset.filter(bemor_id__id=int(bemor)) | queryset.filter(sana=tolov_date) | queryset.filter(
                tolangan_sana=tolov_date)
            if tolandi == 'false':
                queryset = queryset.filter(tolandi=False)
            else:
                queryset = queryset.filter(tolandi=True)
        ...
        for tolov in tolovlar:
            ...
            if serializer.is_valid():
                ...
                for i in tolangan_amount:
                    total += int(i.get('summa'))
                total += serializer.validated_data.get("chegirma")
                summa = serializer.validated_data.get("summa")
                payment_status = {"tolandi": total == summa, "haqdor": total > summa}
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                tolov.tolangan_summa.append(
                    {"sana": str(datetime.date.today()), "summa": -(serializer.validated_data.get('summa'))})
                t = 0
                for i in tolov.tolangan_summa:
                    t += int(i.get("summa"))
                if t == tolov.summa:
            ...
            with transaction.atomic():
                ...
                for i in tolovlar:
                    ...
                    if tolov.tolandi != True:
                        tolov.tolangan_summa.append(
                            {"summa": int(i.get('summa')), "sana": check_to_be_created.get('sana')})
    ...
    def get(self, request):
        ...
        if search:
            queryset = queryset.filter(bemor_id__ism__icontains=search) | queryset.filter(
                bemor_id__familiya__icontains=search) | queryset.filter(
                bemor_id__sharif__icontains=search) | queryset.filter(
